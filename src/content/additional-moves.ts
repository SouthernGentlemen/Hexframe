...
const BLUEPRINTS: MoveBlueprint[] = [
  ...
  { id: 8, key: "rift_uppercut", tags: ["void", "launch", "anti_air"], routeRole: "link", description: "Invulnerable launcher that carries grounded routes into air chains.", level: "mid", damage: 52, startup: 8, active: 5, recovery: 20, box: { x: 8, y: 48, w: 44, h: 70 }, hitstun: 23, blockstun: 13, pushback: 2.8, invulnerable: true, launchVelocityY: 8.5, selfLaunchVelocityY: 7.2, cancelInto: [25, 26, 27, 28] },
  { id: 9, key: "bastion_break", tags: ["physical", "guard_break", "heavy"], routeRole: "cashout", routeFamily: "shock", description: "Slow blow with one-hit hyper armor through its active frames.", level: "overhead", damage: 66, startup: 14, active: 4, recovery: 22, box: { x: 24, y: 50, w: 70, h: 38 }, hitstun: 25, blockstun: 18, pushback: 5.2, armorHits: 1 },
  ...
  { id: 11, key: "ashen_sweep", tags: ["fire", "burn", "low"], routeRole: "link", description: "Low flame sweep that punishes standing guard.", level: "low", damage: 36, startup: 8, active: 4, recovery: 14, box: { x: 18, y: 6, w: 72, h: 22 }, hitstun: 18, blockstun: 11, pushback: 3.6 },
  ...
  { id: 18, key: "phoenix_drive", tags: ["fire", "burn", "launch"], routeRole: "cashout", description: "Blazing rising drive that launches grounded routes.", level: "mid", damage: 55, startup: 9, active: 5, recovery: 22, box: { x: 14, y: 38, w: 60, h: 78 }, hitstun: 24, blockstun: 14, pushback: 4.5, lunge: 2.2, launchVelocityY: 7.4 },
  { id: 19, key: "permafrost", tags: ["cold", "freeze", "control"], routeRole: "cashout", description: "Dense cold wave designed for lock-down.", level: "low", damage: 42, startup: 12, active: 7, recovery: 17, box: { x: 20, y: 8, w: 96, h: 34 }, hitstun: 23, blockstun: 15, pushback: 3.0 },
  ...
  { id: 25, key: "astral_jab", tags: ["void", "air", "strike"], routeRole: "link", description: "Fast airborne check that routes into every air branch.", level: "mid", damage: 24, startup: 4, active: 3, recovery: 8, box: { x: 10, y: 26, w: 54, h: 24 }, hitstun: 14, blockstun: 8, pushback: 1.8, airOk: true, staminaCost: 8, cancelInto: [26, 27, 28] },
...
function cancelTargets(blueprint: MoveBlueprint): number[] {
  if (blueprint.cancelInto) return blueprint.cancelInto;
...
function buildMove(blueprint: MoveBlueprint): RawMove {
  ...
  return {
    ...
    airOk: blueprint.airOk ?? false,
    staminaCost: blueprint.staminaCost ?? (
      blueprint.routeRole === "starter" ? 8 :
      blueprint.routeRole === "link" ? 12 :
      blueprint.routeRole === "reversal" ? 24 : 18
    ),
    hitboxes: [{
      id: 1,
      box: blueprint.box,
    ...
    invulWindows: blueprint.invulnerable
      ...
      : [],
    armorWindows: blueprint.armorHits
      ? [{ startFrame: 0, endFrame: activeEnd, hits: blueprint.armorHits }]
      : [],
    movement: blueprint.selfLaunchVelocityY
      ? [{ frame: 0, vx: blueprint.lunge ?? 0, vy: blueprint.selfLaunchVelocityY }]
      : blueprint.lunge
        ? [{ frame: 0, vx: blueprint.lunge, vy: blueprint.airOk ? -2.8 : 0 }, { frame: activeStart, vx: 0, vy: blueprint.airOk ? -4.2 : 0 }]
        : blueprint.key === "meteor_heel"
          ? [{ frame: activeStart, vx: 1.2, vy: -5.4 }]
          : [],
