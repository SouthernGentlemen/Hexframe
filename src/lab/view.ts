...
export function buildLabView({ character, buildState, preferences, dummyOptions, publicPlay = false }: LabViewOptions): string {
  ...
  return `<a class="skip-link" href="#game-content">Skip to combat lab</a>
  <main class="lab-shell" id="game-content">
    <header class="lab-header debugger-header"><div class="brand"><p class="eyebrow">HEXFRAME / DETERMINISTIC COMBAT LAB V0</p><h1>Combat operating system.</h1><p>Every panel describes one authoritative frame.</p></div><div class="header-actions"><span class="controller-state" id="controller-state">Keyboard ready</span><button type="button" data-action="menu" aria-haspopup="dialog">Armory & settings</button></div></header>

    <section class="frame-console" aria-label="Frame transport controls">
    ...
    <section class="debugger-workspace">
      ...
      <aside class="frame-inspector" id="frame-inspector" aria-label="Authoritative frame inspector"></aside>
    </section>

    <section class="geometry-controls" aria-label="Combat geometry visibility"><strong>GEOMETRY</strong>${[["hitboxes", "Hitboxes"], ["hurtboxes", "Hurtboxes"], ["pushboxes", "Pushboxes"], ["origins", "Origins"], ["skeleton", "Skeleton"]].map(([key, label]) => `<label><input type="checkbox" data-debug="${key}"> ${label}</label>`).join("")}</section>
    ...
    <footer class="control-legend"><span><b>MOVE</b> WASD / left stick</span><span><b>ACTIONS</b> arrows / Y X B A</span><span><b>BANK 2</b> Shift / LT</span><span><b>BANK 3</b> E / RT</span><span><b>SCRUB</b> , and . / LB RB</span><span><b>PLAY</b> Space / Start</span><span><b>MENU</b> Esc / View</span></footer>
    ...
    <div class="menu-scrim" id="menu-scrim" hidden><aside class="lab-menu" id="lab-menu" role="dialog" aria-modal="true" aria-labelledby="menu-title" tabindex="-1">
      ...
      <nav class="menu-tabs" role="tablist" aria-label="Armory sections">${menuTab("loadout", "Loadout", true)}${menuTab("armor", "Armor")}${menuTab("craft", "Craft")}${menuTab("status", "Status codex")}${menuTab("settings", "Settings")}${menuTab("training", "Training")}${publicPlay ? "" : menuTab("debug", "Debug")}</nav>
