...
export function buildLabView({ character, buildState, preferences, dummyOptions, publicPlay = false, gameMode, session = null, save, developerTools = !publicPlay, unlockedMoveIds, unlockedRecipeIds }: LabViewOptions): string {
  ...
  <main class="lab-shell${publicPlay ? " public-play" : ""}" id="game-content">
    <header class="lab-header debugger-header"><div class="brand"><p class="eyebrow">${developerTools ? "HEXFRAME / TRAINING / DEVELOPER TOOLS" : `HEXFRAME / ${(gameMode ?? "game").toUpperCase()}`}</p><h1>${developerTools ? "Combat operating system." : "Prime. Link. Cash out."}</h1><p>${developerTools ? "Every panel describes one authoritative frame." : "Build a sixteen-technique arsenal. Route statuses. Finish the fight."}</p></div><div class="header-actions"><span class="controller-state" id="controller-state">Keyboard ready</span><button type="button" data-action="menu" aria-haspopup="dialog">Pause</button></div></header>

    ${frontShell(buildState, save, session)}
    ...
    <section class="debugger-workspace">
      <section class="playfield-card" aria-label="Combat arena">
        <div class="hud" aria-label="Fighter health and stamina"><div class="hud-player"><span>P1</span><div class="hud-meter-stack"><div class="health-track"><i id="health-p1"></i></div><div class="stamina-track"><i id="stamina-p1"></i></div></div><strong><b id="health-text-p1">${character.health}</b><small id="stamina-text-p1">${character.stamina} STA</small></strong></div><div class="hud-player hud-player-right"><strong><b id="health-text-p2">${bossMode ? 1800 : 1000}</b><small id="stamina-text-p2">${bossMode ? "THE BELL WARDEN" : "100 STA"}</small></strong><div class="hud-meter-stack"><div class="health-track"><i id="health-p2"></i></div><div class="stamina-track"><i id="stamina-p2"></i></div></div><span>${bossMode ? "BOSS" : "DUMMY"}</span></div></div>
        ${campaignMode ? `<aside class="campaign-hud" id="campaign-hud" aria-live="polite"><small>BLACK BELFRY</small><strong id="campaign-objective">Reach the Arsenal Shrine</strong><span id="campaign-loot">Iron Scrap 2 · Grave Thread 3</span></aside>` : ""}
    ...
    <div class="menu-scrim" id="menu-scrim" hidden><aside class="lab-menu" id="lab-menu" role="dialog" aria-modal="true" aria-labelledby="menu-title" tabindex="-1">
      ...
      <nav class="menu-tabs" role="tablist" aria-label="Pause menu">${pauseMenuTabs(gameMode, developerTools, session?.options.tutorial === true)}</nav>
...
function frontShell(buildState: BuildState, save: PlayerSave | undefined, session: GameSession | null): string {
  if (session) return "";
  const stage = save?.campaign.stages["black-belfry"];
  const status = (stage?.status ?? "in-progress").replace("-", " ").toUpperCase();
  const checkpoint = stage?.checkpointId ? "Belfry Crossing" : "Belfry Approach";
  const selected = save?.loadouts.activeId ?? `loadout-0${buildState.activePreset + 1}`;
  const loadouts = buildState.presets.map((build, index) => {
    const id = `loadout-0${index + 1}`;
    const routeCounts = [0, 1, 2, 3].map((column) => [0, 1, 2, 3].filter((bank) => build.loadout[bank * 4 + column] !== 0).length);
    const equipped = build.loadout.filter((moveId) => moveId !== 0).length;
    return `<button type="button" class="loadout-select-card${id === selected ? " selected" : ""}" data-front-loadout="${id}" aria-pressed="${id === selected}"><span>LOADOUT ${String(index + 1).padStart(2, "0")}</span><strong>${build.name}</strong><em>${equipped} / 16 TECHNIQUES</em><dl><div><dt>Fire</dt><dd>${"★".repeat(routeCounts[0])}</dd></div><div><dt>Venom</dt><dd>${"★".repeat(routeCounts[1])}</dd></div><div><dt>Frost</dt><dd>${"★".repeat(routeCounts[2])}</dd></div><div><dt>Shock</dt><dd>${"★".repeat(routeCounts[3])}</dd></div></dl></button>`;
  }).join("");
  return `<section class="front-shell" id="front-shell" role="dialog" aria-modal="true" aria-label="Hexframe main menu" data-selected-loadout="${selected}">
    <section class="front-view front-title active" data-front-view="title"><p>WIZARD GANG PRESENTS</p><h1>SHADOW<span>MONEY</span></h1><button type="button" data-front-action="enter-menu">Press any button</button></section>
    <section class="front-view front-main" data-front-view="main" hidden><header><p>HEXFRAME</p><h2>Main menu</h2></header><nav aria-label="Main menu"><button type="button" data-front-destination="campaign"><strong>Campaign</strong><span>Continue the Black Belfry</span></button><button type="button" data-front-destination="fight"><strong>Fight</strong><span>VS CPU</span></button><button type="button" data-front-destination="training"><strong>Training</strong><span>Simulator and frame tools</span></button><button type="button" data-front-destination="armory"><strong>Armory</strong><span>Arsenal · Equipment · Forge</span></button><button type="button" data-front-destination="codex"><strong>Codex</strong><span>Moves · Status · Equipment · Stages · Enemies</span></button><button type="button" data-front-destination="system"><strong>System</strong><span>Tutorial · Settings · Profile · Credits</span></button></nav><footer><span>SAVE REVISION ${save?.revision ?? 0}</span><span>SERVER PERSISTENCE</span></footer></section>
    <section class="front-view front-destination" data-front-view="campaign" hidden><header><button type="button" data-front-action="back">← Main menu</button><p>CAMPAIGN</p><h2>Stage map</h2></header><div class="campaign-map"><ol><li class="active"><i></i><strong>Black Belfry</strong><span>${status}</span></li><li class="locked"><i></i><strong>Stage 02</strong><span>Locked</span></li><li class="locked"><i></i><strong>Stage 03</strong><span>Locked</span></li></ol><article><div class="belfry-preview" aria-label="Black Belfry stage preview"><span>BLACK BELFRY</span><i></i><i></i><i></i></div><p>BLACK BELFRY / CAMPAIGN</p><h3>The bell remembers every debt.</h3><dl><div><dt>Status</dt><dd>${status}</dd></div><div><dt>Checkpoint</dt><dd>${checkpoint}</dd></div><div><dt>Boss</dt><dd>Bell Warden</dd></div><div><dt>Rewards</dt><dd>Grave Toll · Warden's Vambraces</dd></div></dl><div class="front-actions"><button type="button" data-front-action="edit-loadout">Loadout · ${buildState.presets[buildState.activePreset].name}</button><button class="primary" type="button" data-launch-mode="campaign">${stage?.checkpointId ? "Continue" : "Enter stage"}</button><button type="button" data-front-action="new-campaign">New campaign</button></div></article></div></section>
    <section class="front-view front-destination" data-front-view="fight" hidden><header><button type="button" data-front-action="back">← Main menu</button><p>FIGHT / VS CPU</p><h2>Select loadout</h2></header><div class="front-loadout-grid">${loadouts}</div><div class="selection-summary"><span>OPPONENT</span><strong>Bell Warden</strong><span>STAGE</span><strong>Warden Arena</strong><span>HAZARDS</span><strong>Off</strong><button class="primary" type="button" data-launch-mode="fight">Fight</button></div></section>
    <section class="front-view front-destination" data-front-view="training" hidden><header><button type="button" data-front-action="back">← Main menu</button><p>TRAINING</p><h2>Select loadout</h2></header><div class="front-loadout-grid">${loadouts}</div><div class="selection-summary"><span>DUMMY</span><strong>Stand</strong><span>STAGE</span><strong>Training Grid</strong><span>TOOLS</span><strong>Recording · Frame timeline · Display · Scenarios</strong><button class="primary" type="button" data-launch-mode="training">Start training</button></div></section>
  </section>`;
}
...
function pauseMenuTitle(mode: GameMode | undefined, developerTools: boolean): string {
  ...
  if (mode === "fight") return "Match paused";
  if (mode === "campaign") return "Campaign paused";
  return "Game menu";
}

function pauseMenuTabs(mode: GameMode | undefined, developerTools: boolean, tutorial = false): string {
  if (developerTools || mode === "training") return `<span class="menu-tab-group" role="presentation"><small>TRAINING</small>${menuTab("training", "Dummy · Frame Tools", true)}${menuTab("loadout", "Loadout")}${tutorial ? menuTab("moves", "Codex") : ""}${menuTab("settings", "Settings")}${developerTools ? menuTab("debug", "Developer") : ""}</span>`;
  ...
  if (mode === "campaign") return `<span class="menu-tab-group" role="presentation"><small>CAMPAIGN</small>${menuTab("loadout", "Arsenal", true)}${menuTab("armor", "Equipment")}${menuTab("moves", "Codex")}${menuTab("settings", "Settings")}</span>`;
  return `<span class="menu-tab-group" role="presentation"><small>ARMORY</small>${menuTab("loadout", "Arsenal", true)}${menuTab("armor", "Equipment")}${menuTab("craft", "Forge")}</span><span class="menu-tab-group" role="presentation"><small>CODEX</small>${menuTab("moves", "Moves")}${menuTab("status", "Status")}${menuTab("codex-equipment", "Equipment")}${menuTab("stages", "Stages")}${menuTab("enemies", "Enemies")}</span><span class="menu-tab-group" role="presentation"><small>SYSTEM</small>${menuTab("tutorial", "Tutorial")}${menuTab("settings", "Settings")}${menuTab("profile", "Profile")}${menuTab("credits", "Credits")}</span>`;
}

function pauseActions(mode: GameMode | undefined): string {
  if (mode === "fight") return `<button type="button" data-action="reset" data-gamepad-nav>Restart match</button><button type="button" data-action="return-mode-select" data-gamepad-nav>Stage select</button>`;
  if (mode === "training") return `<button type="button" data-action="reset" data-gamepad-nav>Reset training</button><button type="button" data-action="return-main" data-gamepad-nav>Exit training</button>`;
  if (mode === "campaign") return `<button type="button" data-action="reset" data-gamepad-nav>Restart checkpoint</button><button type="button" data-action="return-mode-select" data-gamepad-nav>Leave stage</button>`;
  return "";
}
