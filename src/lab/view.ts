...
export function buildLabView({ character, buildState, preferences, dummyOptions }: LabViewOptions): string {
  ...
  const assignmentRows = ACTION_SLOT_LABELS.map((label) => {
    ...
    return `<label class="loadout-row" data-gamepad-nav tabindex="0" data-action-bank="${Math.trunc(label.slot / 4)}" data-move-preview="${moveId}">
  ...
  <main class="lab-shell" id="game-content">
    <header class="lab-header"><div class="brand"><p class="eyebrow">HEXFRAME / PROTOTYPE</p><h1>Build your route.</h1></div><div class="header-actions"><span class="controller-state" id="controller-state">Keyboard ready</span><button class="primary" type="button" data-action="pause">Pause</button><button type="button" data-action="menu" aria-haspopup="dialog">Armory & settings</button></div></header>
    <section class="playfield-card" aria-label="Combat arena">
      <div class="hud" aria-label="Fighter health"><div class="hud-player"><span>YOU</span><div class="health-track"><i id="health-p1"></i></div><strong id="health-text-p1">${character.health}</strong></div><div class="frame-readout"><span id="play-state">LIVE</span><strong id="frame-readout">0</strong></div><div class="hud-player hud-player-right"><strong id="health-text-p2">1000</strong><div class="health-track"><i id="health-p2"></i></div><span>DUMMY</span></div></div>
    ...
    <div class="menu-scrim" id="menu-scrim" hidden><aside class="lab-menu" id="lab-menu" role="dialog" aria-modal="true" aria-labelledby="menu-title" tabindex="-1">
      ...
      <nav class="menu-tabs" role="tablist" aria-label="Armory sections">${menuTab("loadout", "Loadout", true)}${menuTab("gear", "Gear")}${menuTab("status", "Status codex")}${menuTab("settings", "Settings")}${menuTab("training", "Training")}${menuTab("debug", "Debug")}</nav>
      ...
      <section class="menu-page active armory-page" id="page-loadout" role="tabpanel" aria-labelledby="tab-loadout" data-menu-page="loadout">
        <div class="armory-titlebar"><div><p class="eyebrow">ACTION MAP / <span data-build-number>BUILD ${String(buildState.activePreset + 1).padStart(2, "0")}</span></p><h2>Loadout</h2></div>${presets}</div>
        <div class="loadout-workspace"><section class="input-sheet" aria-labelledby="input-sheet-title"><div class="panel-heading"><div><small>INPUT SCHEMATIC</small><h3 id="input-sheet-title">Keyboard + gamepad</h3></div><span>16 MAPPED</span></div>${deviceOutlines()}<div class="mapped-inputs">${inputButtons}</div></section><aside class="assignment-sheet" aria-labelledby="assignment-title"><div class="panel-heading"><div><small>ACTION DECK</small><h3 id="assignment-title">Assigned moves</h3></div><button type="button" data-action="default-loadout" data-gamepad-nav>Reset</button></div><div class="loadout-grid">${assignmentRows}</div></aside></div>
      ...
      <section class="menu-page gear-page" id="page-gear" role="tabpanel" aria-labelledby="tab-gear" data-menu-page="gear" hidden>
        <div class="armory-titlebar"><div><p class="eyebrow">INVENTORY / <span data-build-number>BUILD ${String(buildState.activePreset + 1).padStart(2, "0")}</span></p><h2>Gear</h2></div>${presets}</div>
        <div class="gear-workspace"><aside class="character-sheet" aria-labelledby="character-sheet-title"><div class="character-crest"><span aria-hidden="true">SW</span><div><small>LEVEL 1 · OCCULTIST</small><h3 id="character-sheet-title">${preset.name}</h3></div></div><div class="stat-runes" aria-label="Build statistics"><div><small>VIT</small><strong id="stat-vitality">${Math.trunc(character.health / 100)}</strong></div><div><small>POW</small><strong>12</strong></div><div><small>ARC</small><strong>16</strong></div><div><small>CTL</small><strong>14</strong></div></div><h4>Equipped gear</h4><div class="equipment-grid">${equipment}</div></aside><section class="inventory-sheet" aria-labelledby="inventory-title"><div class="panel-heading"><div><small>VAULT · ${GEAR_CATALOG.length} ITEMS</small><h3 id="inventory-title">Inventory</h3></div></div><div class="inventory-grid">${inventory}</div><div class="gear-detail" id="gear-detail" aria-live="polite"><span class="gear-icon" aria-hidden="true">${initialGear.icon}</span><div><small>${initialGear.rarity} ${initialGear.slot}</small><h4>${initialGear.name}</h4><p>${initialGear.description}</p></div><ul>${initialGear.tags.map((tag) => `<li>${tag}</li>`).join("")}</ul></div></section></div>
      </section>

      <section class="menu-page codex-page" id="page-status" role="tabpanel" aria-labelledby="tab-status" data-menu-page="status" hidden><div class="page-intro"><div><p class="eyebrow">COMBO LOGIC</p><h2>Prime. Link. Cash out.</h2></div></div><div class="status-rules">${statusCards}</div><div class="route-examples"><h3>Starter routes</h3><div><article><b>IGNITE LOOP</b><span>Ember Palm → Ashen Sweep → Phoenix Drive</span></article><article><b>VENOM ENGINE</b><span>Venom Fang → Toxic Bloom → Plague Touch</span></article><article><b>VOLTAGE CASHOUT</b><span>Storm Knuckle → Static Rush → Bastion Break</span></article><article><b>BLEED EXECUTE</b><span>Crimson Arc → Blood Moon → Reaper Kick</span></article></div></div></section>
